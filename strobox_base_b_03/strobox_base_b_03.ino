...
float bpm = 140;
float tempoBatida = (60.00*941175.907)/bpm; // um pulso em micro segundos Esse valor foi atingido ao fazer uma media 941175.907
...
float escalaMiMenor[] = {16.352,  18.354,  20.602,  23.125,  24.500,  27.500,  30.868,
                         ...
                         1046.5,  1174.7,  1318.5,  1480.0,  1568.0,  1760.0,  1975.5,
...
//                             C#/Db    D        E        F#/Gb    G        G#/Ab    A        B

float escalaBluesMiMenor[] =  {17.324,  18.354,  20.602,  23.125,  24.500,  25.957,  27.500,  30.868,
                               34.648,  36.708,  41.203,  46.249,  48.999,  51.913,  55.000,  61.735,
                               69.296,  73.416,  82.407,  92.499,  97.999,  103.83,  110.00,  123.47,
                               138.59,  146.83,  164.81,  185.00,  196.00,  207.65,  220.00,  246.94,
                               277.18,  293.66,  329.63,  369.99,  392.00,  415.30,  440.00,  493.88,
                               554.37,  587.33,  659.26,  739.99,  783.99,  830.61,  880.00,  987.77,
                               1108.7,  1174.7,  1318.5,  1480.0,  1568.0,  1661.2,  1760.0,  1975.5,
                               2217.5,  2349.3,  2637.0,  2960.0,  3136.0,  3322.4,  3520.0,  3951.1,
                               4434.9,  4698.6,  5274.0,  5919.9,  6271.9,  6644.9,  7040.0,  7902.1};
...
void Tocar() {
  if (tocar == 1) {
    ...
    switch (numeroEscala) {
        ...
        case 0:
        tone(pinoSom, (escalaMiMenor[map(analogRead(pot1), 0, 1023, 0, (sizeof(escalaMiMenor)/sizeof(float)))]) ,40);
        break;
        
        case 1:
        tone(pinoSom, (escalaPentaMiMenor[map(analogRead(pot1), 0, 1023, 0, (sizeof(escalaPentaMiMenor)/sizeof(float)))]) ,40);
        break;
      
        case 2:
        tone(pinoSom, (escalaBluesMiMenor[map(analogRead(pot1), 0, 1023, 0, (sizeof(escalaBluesMiMenor)/sizeof(float)))]) ,40);
        break;
    }
  }
  tocar = 0;
...
void ControleEscala() {
  ...
  numeroEscala = map(analogRead(pot2), 0, 1023, 0, 3);
}
...
void controleEscala() {
  ...
  if (numeroEscala != numeroEscalaAnterior) {
    ...
    switch (numeroEscala) {
      case 1:
        tamanhoEscala = sizeof(escalaMenorNatural)/sizeof(byte);
        for(int i = 0; i > tamanhoEscala; i++) {
         escala[i] = escalaMenorNatural[i];
        }
        break;
        
        case 2:
          tamanhoEscala = sizeof(pentatonicaMenor)/sizeof(byte);
          for(int i = 0; i > tamanhoEscala; i++) {
           escala[i] = pentatonicaMenor[i];
          }
        break;  
